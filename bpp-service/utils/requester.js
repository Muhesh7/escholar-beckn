...
exports.postRequest = async (baseURL, route, headers = {}, body, shouldSign, host='bpp.beckn.captainirs.dev') => {
	try {
		baseURL = baseURL.replace(/\/$/, '')
		let url = baseURL + route
		if (shouldSign) headers = { ...headers, authorization: await createAuthorizationHeader(body, host) }
		// console.log('URL: ', url)
...
exports.externalPOSTRequest = (shouldSign) => {
	return async ({ baseURL, headers, body, route, host }) =>
		await exports.postRequest(baseURL, route, headers, body, shouldSign, host)
}
