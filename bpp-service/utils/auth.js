...
exports.createAuthorizationHeader = async (message, host) => {
	...
	try {
		console.log('Authorization header host', host)
		const user = await User.findOne({ subscriberId:host })
		if (!user) throw new Error('User not found')
		const privateKey = user.privateKey
		const signature = await signMessage(signingString, privateKey || '')
		const subscriberId = user.subscriberId
		const header = `Signature keyId="${subscriberId}|${user.subscriberId}|ed25519",algorithm="ed25519",created="${created}",expires="${expires}",headers="(created) (expires) digest",signature="${signature}"`
		return header
	}	
	catch (err) {
		console.log('Authorization header', err)
		throw new Error('Error getting bpp private key')
	}
