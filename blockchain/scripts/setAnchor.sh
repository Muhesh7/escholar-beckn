...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o orderer.escholar.captainirs.dev:7050 --ordererTLSHostnameOverride orderer.escholar.captainirs.dev -c ${CHANNEL_NAME} --tls --cafile $ORDERER_CA
  { set +x; } 2>/dev/null
